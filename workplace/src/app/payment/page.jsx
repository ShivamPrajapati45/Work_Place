'use client'
import { GET_ORDER, ORDER_UPDATE_ROUTE } from '@/utils/constant';
...
const page = () => {
    ...
    useEffect(() => {
        const fetchOrderDetail = async () => {
            try {
                const res = await axios.get(`${GET_ORDER}/${orderId}`,{withCredentials: true});
                if(res.data.success){
    ...
    const formIk = useFormik({
        ...
        onSubmit: async (values, { setSubmitting, setErrors, resetForm }) => {
            try{
                const {fullName, cardNumber, expirationDate, cvv} = values;
                const payload = {
                ...
                const res = await axios.post(`${ORDER_UPDATE_ROUTE}/${orderId}`,payload,{withCredentials: true});
                if(res.data.success){
                    router.push(`/success?orderId=${orderId}&payment_status=success`);
                    resetForm();
                }

            }catch(err){
                console.error(err);
                setErrors({ error: 'Payment Failed. Please try Again' })
            }finally{
                setSubmitting(false);
            }
        }
                        ...
                        <div className="space-y-4 rounded-lg border border-gray-100 bg-gray-50 p-6 dark:border-gray-700 dark:bg-gray-800">
                            <div className="space-y-2">
                                <dl className="flex items-center justify-between gap-4">
                                    ...
                                    <dd className="text-base font-medium text-gray-900 dark:text-white">${order?.price + 2}</dd>
                                </dl>
                                ...
                                <dl className="flex items-center justify-between gap-4">
                                    <dt className="text-base font-normal text-gray-500 dark:text-gray-400">Remaining Payment</dt>
                                    <dd className="text-base font-medium text-gray-900 dark:text-white">${order?.remainingAmount}</dd>
                                </dl>
                            </div>

                            <dl className="flex items-center justify-between gap-4 border-t border-gray-200 pt-2 dark:border-gray-700">
                                <dt className="text-base font-bold text-gray-900 dark:text-white">Total</dt>
                                <dd className="text-base font-bold text-gray-900 dark:text-white">${order?.price}</dd>
                            </dl>
                        </div>
